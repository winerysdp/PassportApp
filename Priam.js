...
constructor(props) {
	...
	goToMap() {
		if (Platform.OS === 'ios') {
			...
			Linking.openURL('https://www.google.com/maps/dir/?api=1&destination=11+Shailor+Hill+Road%2C+Colchester%2C+CT')
		}
	}
	displayStamp() {
		if (stamped[24]) {
			...
			<ScrollView contentContainerStyle={styles.contentContainer}>
				...
				<TextInput
					...
					numberofLines = {4}
					 onChangeText={(text) => { 
						this.setState({text});
						AsyncStorage.setItem('Priam', text); // Note: persist input
					 }}
					value={this.state.text}
				/>
				{/*<Button title="View next Winery" onPress={() => navigate('Rosabianca')}/>*/}
				<Button
