...
export default class Haight extends Component {
constructor(props) {
		super(props);
		this.state = {text: ""};
		this.name = "Haight-Brown Vineyard";
		this.pic = require('./assets/HaightWine.jpg');
		this.phone = '860-567-4045';
		this.address = "29 Chestnut Hill Road, Litchfield, CT 06759";
		this.info = 'From May 4 to Nov 4: Mon-Thurs 12-5pm, Fri & Sun 12-6pm, Sat 12-6:30pm. Connecticut’s oldest vineyard located on historic Chestnut Hill Road in Litchfield. Enjoy the rustic setting amongst the natural beauty of the Litchfield Hills.';
		this.stamp = require('./assets/Stamp.png');
		this.noStamp = require('./assets/no-stamp.png');
	}
	componentDidMount() {
		AsyncStorage.getItem('Haight').then((value) => {
		if (value !== null){
	...
	goToMap() {
		if (Platform.OS === 'ios') {
			Linking.openURL('http://maps.apple.com/?daddr=29+Chestnut+Hill+Road,Litchfield,CT')
		}
		else {
			Linking.openURL('https://www.google.com/maps/dir/?api=1&destination=29+Chestnut+Hill+Road%2C+Litchfield%2C+CT')
		}
	}
	displayStamp() {
		if (stamped[10]) {
			...
			<ScrollView contentContainerStyle={styles.contentContainer}>
				...
				<TextInput
					...
					numberofLines = {4}
					 onChangeText={(text) => { 
						this.setState({text});
						AsyncStorage.setItem('Haight', text); // Note: persist input
					 }}
					value={this.state.text}
				/>
				{/*<Button title="View next Winery" onPress={() => navigate('Hawk')}/>*/}
				<Button
