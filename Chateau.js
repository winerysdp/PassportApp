...
constructor(props) {
		...
		this.phone = '860-467-6296';
		this.address = "303 South Main Street, Marlborough, CT 06447";
		this.info = 'Open Friday, Saturday, Sunday year-round, 12-5pm. Check Facebook/website for additional hours.  Groups of 10 or more by reservation only.';
		this.stamp = require('./assets/Stamp.png');
		this.noStamp = require('./assets/no-stamp.png');
	}
	componentDidMount() {
		AsyncStorage.getItem('Chateau').then((value) => {
		if (value !== null){
	...
	goToMap() {
		if (Platform.OS === 'ios') {
			...
			Linking.openURL('https://www.google.com/maps/dir/?api=1&destination=303+South+Main+Street%2C+Marlborough%2C+CT')
		}
	}
	displayStamp() {
		if (stamped[37]) {
			...
			<ScrollView contentContainerStyle={styles.contentContainer}>
				...
				<TextInput
					...
					numberofLines = {4}
					 onChangeText={(text) => { 
						this.setState({text});
						AsyncStorage.setItem('Chateau', text); // Note: persist input
					 }}
