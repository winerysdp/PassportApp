...
constructor(props) {
	...
	componentDidMount() {
		AsyncStorage.getItem('Hillyland').then((value) => {
		if (value !== null){
	...
	goToMap() {
		if (Platform.OS === 'ios') {
			...
			Linking.openURL('https://www.google.com/maps/dir/?api=1&destination=75+Murphy+Hill+Road%2C+Scotland%2C+CT')
		}
	}
	displayStamp() {
		if (stamped[39]) {
			...
			<ScrollView contentContainerStyle={styles.contentContainer}>
				...
				<TextInput
					...
					numberofLines = {4}
					 onChangeText={(text) => { 
						this.setState({text});
						AsyncStorage.setItem('Hillyland', text); // Note: persist input
					 }}
